...
import {
  CheckedInAttendeeCard,
  AttendeeRow,
} from "../components/EventDirectory";
...
export default function Live({ currentUser }) {
  ...
  if (loading) {
    return (
      <div
        className="main-feed"
        style={{ padding: 48, textAlign: "center", minHeight: "100dvh" }}
      >
    ...
    return (
      ...
      >
        ...
        <p style={{ color: "var(--text-secondary)", maxWidth: 400 }}>
          There is no active event right now. The live directory will be
          available here during our next meetup.
        </p>
      </div>
    );
  }
  ...
  const { start: eventStartTime } = parseMeetupTimes(
    activeMeetup.date,
    activeMeetup.time,
  );
  ...
  return (
    <div
      className="main-feed"
      style={{ padding: "24px 16px", minHeight: "100dvh", paddingBottom: 100 }}
    >
      <div
        style={{
          display: "flex",
          ...
          >
            ...
            >
              {isEventStarted ? (
                ...
                <Lock size={12} style={{ display: "inline", marginTop: -2 }} />
              )}
            </span>
          </div>
          ...
          {!isEventStarted ? (
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
              {checkedInReservations.map((r) => (
                <CheckedInAttendeeCard key={r.id} reservation={r} />
              ))}
            </div>
          )}
        </div>
