...
function MeetupForm({ onSave, initial, onCancel }) {
  const getInitialForm = () => {
    ...
    let rawDate = "",
      startTime = "",
      endTime = "";
    if (initial.date && initial.time) {
      const { start, end } = parseMeetupTimes(initial.date, initial.time);
    ...
    return { ...initial, rawDate, startTime, endTime };
  };
  ...
  const getFormattedDate = (dateStr) => {
    ...
    const suffix =
      ["th", "st", "nd", "rd"][((dNum % 100) - 20) % 10] ||
      ["th", "st", "nd", "rd"][dNum % 100] ||
      "th";
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true);
    try {
      const finalDate = form.rawDate
        ? getFormattedDate(form.rawDate)
        : form.date;
      const finalTime =
        form.startTime && form.endTime
          ? `${formatAMPM(form.startTime)} – ${formatAMPM(form.endTime)} (IST)`
          : form.time;
      ...
      const payload = {
        ...
        price: parseInt(form.price) || 422,
      };

      if (initial?.id) {
        await updateMeetup(initial.id, payload);
      } else {
      ...
      setError(
        err?.response?.data?.detail || err?.message || "Failed to save meetup.",
      );
      setSaving(false);
    }
  };
    ...
    >
      ...
      >
        <div>
          ...
          <input
            ...
            placeholder="Perenti 29th Meetup"
            style={inputSt}
      ...
      <div>
        ...
        <input
          ...
          placeholder="Birch Cafe Vanasthalipuram, Hyderabad"
          style={inputSt}
          onFocus={(e) => (e.target.style.borderColor = "var(--primary)")}
          onBlur={(e) => (e.target.style.borderColor = "var(--border)")}
        />
      ...
      <div>
        ...
        >
          Recommended: 1200x630px, under 2MB (JPG, PNG, WebP)
        </div>
      ...
      <div style={{ display: "flex", gap: 10 }}>
        ...
        >
          {saving ? "Saving…" : initial ? "Update Meetup" : "Create Meetup"}
        </button>
      </div>
    </form>
  );
...
function MeetupAdminCard({ meetup, onRefresh, onScanClick }) {
  ...
  // Compute if event is completed or started
  let isCompleted = false;
  let isStarted = false;
  try {
    const { start, end } = parseMeetupTimes(
      meetup.date || "",
      meetup.time || "",
    );
    if (now > end.getTime()) {
  ...
  let pillText = "Hidden";
  let pillBg = "var(--bg-elevated)";
  let pillColor = "var(--text-tertiary)";
  ...
  if (meetup.is_active) {
    if (isCompleted) {
      ...
      pillText = "Active";
      pillBg = "var(--primary-glow)";
      pillColor = "var(--primary)";
      pillBorder = "rgba(3,212,124,0.2)";
    }
  }
        ...
        {editing ? (
          ...
          <>
            ...
            >
              <div
                style={{
                  ...
                  width: `${Math.min(100, (totalAttendees / (meetup.capacity || 1)) * 100)}%`,
            ...
            >
              ...
              >
                <Users size={14} />
                {expanded ? "Hide" : "Show"} {reservations.length} registration
  ...
  const processTicket = async (ticketId) => {
    ...
    try {
      ...
      if (res.status === "success") {
        const isWrongEvent =
          scanMeetupId && res.reservation?.meetup_id !== scanMeetupId;
  ...
  return (
    ...
    >
      ...
      >
        <button
          ...
          style={{
            ...
            border: "none",
            background:
              action === "check_in" ? "var(--primary)" : "transparent",
        ...
        <button
          ...
          style={{
            ...
            border: "none",
            background:
              action === "check_out" ? "var(--primary)" : "transparent",
      ...
      {result && (
        <div
          style={{
            background: result.isWrongEvent
              ? "rgba(242,87,48,0.06)"
              : "rgba(3,212,124,0.06)",
        ...
        >
          <div
            style={{
              ...
              height: 44,
              borderRadius: "50%",
              background: "rgba(242,87,48,0.15)",
              display: "flex",
              alignItems: "center",
              justifyContent: "center",
              margin: "0 auto 12px",
      ...
      >
        <form onSubmit={handleManualSubmit}>
          ...
          <div style={{ display: "flex", gap: 10 }}>
            <input
              type="text"
              placeholder="e.g. PRNT-Perenti-ABC123"
              value={manualId}
...
function PendingApprovalsTab({ session }) {
  ...
  const handleApprove = async (id) => {
    ...
    try {
      ...
      alert(
        "Approval failed: " + (err?.response?.data?.detail || err?.message),
      );
    } finally {
      setActioning(null);
    }
  ...
  const handleReject = async (id) => {
    if (
      !window.confirm(
        "Reject this registration? The user will need to re-register.",
      )
    )
      return;
    setActioning(id);
    try {
      ...
      alert(
        "Rejection failed: " + (err?.response?.data?.detail || err?.message),
      );
    } finally {
      setActioning(null);
    }
  ...
  const filtered = items.filter((r) => {
    const nameMatch =
      !nameQuery ||
      (r.user_name || "").toLowerCase().includes(nameQuery.toLowerCase());
    const eventMatch =
      !eventQuery ||
      (r.meetup?.title || "").toLowerCase().includes(eventQuery.toLowerCase());
    const statusMatch =
      statusFilter === "all" ||
      (statusFilter === "pending" && r.status === "pending_payment") ||
      (statusFilter === "expired" && r.status === "rejected");
            ...
            return (
              <div
                ...
                style={{
                  background: "var(--bg-card)",
                  borderRadius: 16,
                  border: `1.5px solid ${isClosed ? "var(--border)" : "rgba(255,113,1,0.3)"}`,
                  overflow: "hidden",
                  ...
                  {isClosed ? (
                    ...
                    <span
                      style={{
                        ...
                        padding: "3px 10px",
                        borderRadius: 999,
                        background: "rgba(255,113,1,0.15)",
                        color: "var(--orange, #FF7101)",
                        textTransform: "uppercase",
                        letterSpacing: "0.05em",
                      }}
                  ...
                  {!isClosed && (
                    <div style={{ display: "flex", gap: 10, marginTop: 6 }}>
                      ...
                      <button
                        ...
                        style={{
                          color: "var(--red)",
                          borderColor: "rgba(242,87,48,0.3)",
                          flex: 1,
                          justifyContent: "center",
                          opacity: isActioning ? 0.6 : 1,
                        }}
  ...
  if (!isAdmin) {
    return (
      ...
      >
        ...
        <div style={{ color: "var(--text-secondary)", maxWidth: 380 }}>
          This panel is only available to administrators. Log in with an admin
          account.
        </div>
        <div
          style={{
      ...
      >
        ...
        <button
          ...
          style={{
            ...
            color:
              tab === "scan" ? "var(--text-primary)" : "var(--text-tertiary)",
            fontWeight: 700,
            fontSize: "0.875rem",
            cursor: "pointer",
            transition: "all 0.15s",
        ...
        {tab === "meetups" ? (
          <>
            ...
            >
              ...
              <StatCard
                label="Active Events"
                value={meetups.filter((m) => m.is_active).length}
                color="var(--primary)"
              />
              <StatCard
                label="Total Capacity"
                value={meetups.reduce((s, m) => s + (m.capacity || 0), 0)}
                color="var(--blue)"
              />
            </div>
